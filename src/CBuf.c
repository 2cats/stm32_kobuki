...
*/
void CBuf_Init(CBuf*cb)//初始化
{
	cb->sp=cb->ep=0;
}
int CBuf_Ava(CBuf*cb)//获取缓存区可用数据长度
{
	return cb->ep-cb->sp;
}
unsigned char CBuf_Pop(CBuf*cb)//弹出缓存区里的一个字节（该数据会被移出缓存区）
{
	unsigned char val;
...
unsigned char CBuf_Get(CBuf*cb,int start)//获取缓存区里的一个字节，并且可带偏移（该数据不会被移出缓存区）
{
	unsigned char val;
